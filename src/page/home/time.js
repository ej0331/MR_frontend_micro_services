...
import Modal from "../../component/chart/hometime";
import "./table.css";
import ExcelJS from "exceljs";
import { getTimeLimitedPracticeChartData } from "../../api/timeLimitedPractice";
import { TimeLimitedPracticelist } from "../../api/excel";
...
const TimeTab = ({
  ...
  const fetchChartData = async (id) => {
    try {
      const res = await getTimeLimitedPracticeChartData(id, startDate, endDate);
  ...
  const handleDownload = async () => {
    try {
      const res = await TimeLimitedPracticelist();
      // 創建一個新的工作簿
