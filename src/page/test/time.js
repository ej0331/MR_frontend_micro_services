...
import Modal from "../../component/chart/testtime";
import "./table.css";
import ExcelJS from "exceljs";
import { getTimeLimitedTestChartData } from "../../api/timeLimitedTest";
import { TimeLimitedTestlist } from "../../api/excel";
...
const TimeTab = ({
  ...
  const fetchChartData = async (id) => {
    try {
      const res = await getTimeLimitedTestChartData(id, startDate, endDate);
      res.data.data.level1_time_limit_list =
        res.data.data.level1_time_limit_list.map((item) => item / 60);
      res.data.data.level1_time_list = res.data.data.level1_time_list.map(
        (item) => item / 60
      );
  ...
  const handleDownload = async () => {
    try {
      const res = await TimeLimitedTestlist();
      // 創建一個新的工作簿
        ...
        <table class="w-full text-sm text-left rtl:text-right text-textcolor dark:text-textcolor ">
          ...
          >
            <tr className="relative">
              ...
              <th scope="col" class="px-3 w-36 font-black text-base">
                第一關-對
              </th>
              <th scope="col" class="px-3 w-36 font-black text-base">
                第一關-錯
              </th>
              <th scope="col" class="px-3 w-44 font-black text-base">
                第一關-總
              </th>
              <th scope="col" class="px-3 w-44 font-black text-base">
                第一關-耗時
              ...
              <th scope="col" class="px-3 w-96 font-black text-base">
                完成日期
              </th>
            </tr>
          </thead>
          <tbody>
            {data.length != 0 ? (
              ...
              <tr class="font-bold text-lg text-center text-darkgray py-20">
                <td class="p-8 bg-maincolor" colSpan={20}>
                  查無資料
                </td>
              </tr>
            )}
  ...
  return <div className="h-full">{TimeTable}</div>;
};
